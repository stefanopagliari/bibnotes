...
export class importAllBib extends Modal {
    ...
    onOpen() {
      ...
      console.log("This bib file has " + bibtex_Array.length + " entries: " + bibtex_Array)
    ...
    for (let EntryNumber = 0; EntryNumber < bibParsed.entries.length; EntryNumber++) {
  ...
  export class fuzzySelectEntryFromBib extends FuzzySuggestModal<referenceSelection> {
    ...
    // Perform action on the selected suggestion.
     onChooseItem(referenceSelected: referenceSelection, evt: MouseEvent | KeyboardEvent) {
       new Notice(`Selected ${referenceSelected.citeKey}`);
